package com.ui
{
	...
	public class newestVideoClip extends MovieClip
	{


		private var _source:String;
		private var _HideVideo:Boolean = true;
		...
		public function newestVideoClip()
		{
			// constructor code
		...
		private function VideoEnd(event:VideoEvent):void
		{
			...
			player_mc.removeEventListener(VideoEvent.COMPLETE, VideoEnd);
			player_mc.removeEventListener(VideoEvent.STATE_CHANGE, checkplaying);
			stopVideo();
			dispatchEvent(new Event(Event.COMPLETE));
		...
		{
			player_mc.addEventListener(VideoEvent.COMPLETE, VideoEnd);
			player_mc.play();
			player_mc.alpha = 1;
			if(rewind == true)
			{
		...
		public function set Mask(m:Boolean):void
		{
			_Mask = m;
			if(m == true)
			{
				this.gotoAndPlay("mask");
			}
			else
			{
				this.gotoAndPlay("no_mask");
			}
		}
